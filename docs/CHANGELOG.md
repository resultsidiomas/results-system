...
Por que: usuario reportou (a) fotos demais da tabela indo pro lead — pediu so a que tem todos os valores; (b) numero de teste `5548996667822@s.whatsapp.net` sem resposta; (c) "\n" aparecendo cru nas mensagens.

Arquivos: backend/src/whatsapp/uazapi/price-table.assets.ts, backend/agents/commercial/prompt-v1.md, backend/src/shared/text-sanitizer.ts (novo), backend/src/agents/commercial/commercial.service.ts. Workflow n8n alterado via `PUT /api/v1/workflows/:id` (nao versionado no repo, mesma limitacao das entradas anteriores) — nodes `Filtro Numero Teste` e `Quebrar Resposta em Blocos`.

Impacto: typecheck limpo (`npx tsc --noEmit`). Tabela de precos agora sempre a mesma imagem (tabela-01), independente do que qualquer chamador passar como variant. Numero de teste `5548996667822@s.whatsapp.net` confirmado na allowlist do workflow (`versionCounter: 44`). Nao testado ainda ponta a ponta contra WhatsApp real apos essas mudancas (pendente confirmar no proximo teste real: pergunta particular/turma antes da tabela, e ausencia de "\n" cru).
